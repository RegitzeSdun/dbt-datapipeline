...
with StartDate AS (
 ...
 FROM {{ref('stg_segment_ios__community_viewed_article')}} AS segment
),
...
historical_events AS (
    ...
    UNION ALL 
    SELECT slug, original_timestamp, category, user_id, referrer 
    FROM {{ref('stg_segment_ios__viewed_article')}} AS segment
    UNION ALL
    SELECT slug, original_timestamp, category, user_id, referrer 
    FROM {{ref('stg_mixpanel__agg_viewed_article')}} AS mixpanel
...
historical_events_with_article_id AS (
    ...
    LEFT JOIN {{source('raw_airbyte__wawa_production_database','community_article')}} AS metadata ON he.slug = metadata.slug
),
...
current_events_with_article_id AS (
    ...
    FROM {{ref('stg_segment_ios__screens_article')}} AS segment
    UNION ALL
    SELECT slug, original_timestamp, category, user_id, referrer, article_id 
    FROM {{ref('stg_segment_android__screens_article')}} AS segment
),
...
WHERE article_id IS NOT NULL and user_id IS NOT NULL
